...
@group(0) @binding(0) var<uniform> compUniforms : CompUniforms;
...
@group(1) @binding(0) var colorTexSampler : sampler;
...
fn frag_main(@location(0) uv : vec2f) -> @location(0) vec4f {
    ...
    let viewportSize = compUniforms.viewportSize;

    //var st : vec2f = uv * 2. - 1.;
    //st = st * (viewportSize / max(viewportSize.x, viewportSize.y)) * (min(colorTexSize.x, colorTexSize.y) / colorTexSize );
    //st = st * 0.5 + 0.5;
